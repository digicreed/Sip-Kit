...
//   <service android:name=".SipKitConnectionService"
//            android:permission="android.permission.BIND_TELECOM_CONNECTION_SERVICE">
//     <intent-filter>
//       <action android:name="android.telecom.ConnectionService" />
//     </intent-filter>
//   </service>
//   <service android:name=".SipKitForegroundService"
...
class SipKitPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    private fun makeCall(call: MethodCall, result: Result) {
        ...
        // TelecomManager: placeCall via ConnectionService
        if (hasTelecomPermission()) {
            val telecomManager = context.getSystemService(Context.TELECOM_SERVICE) as TelecomManager
            val extras = android.os.Bundle().apply {
                putBoolean(TelecomManager.EXTRA_START_CALL_WITH_VIDEO_STATE, video)
            }
            // In a full impl: telecomManager.placeCall(Uri.parse("sip:$target"), extras)
        }
    ...
    private fun holdCall(call: MethodCall, result: Result, hold: Boolean) {
        ...
        emitEvent(mapOf("type" to "callState", "callId" to callId, "state" to if (hold) "held" else "established"))
        result.success(null)
    }
    ...
    // ─── Incoming call (called from PJSUA2 callback) ──────────────────────
    fun onIncomingCall(callId: String, accountId: String, remoteUri: String, displayName: String) {
        if (hasTelecomPermission()) {
            val telecomManager = context.getSystemService(Context.TELECOM_SERVICE) as TelecomManager
            val extras = android.os.Bundle().apply {
